...
PARAMS = dict(
    ...
    # --- Неопт. режим, критерий периодического фонда (v10.4) ---
    neopt_stop_n           = 3,    # остановок: гейт — БОЛЕЕ N остановок за скользящее окно (или циклич. режим)
    neopt_stop_win_d       = 10,   # сут: окно подсчёта остановок для гейта
    neopt_accum_rise_bar   = 5.0,  # бар: рост Рпр за цикл остановки БОЛЕЕ порога -> неопт (стоит с потенциалом; обсуждаемо)
    neopt_ip_over_cas_bar  = 15.0, # бар: Рпр выше Рзатр + столько (обсуждаемо)
    ...
    umch_env_net           = 1.0,  # Гц: мин. снижение верхней огибающей за окно
...
def det_rpl_vogel(ctx, nur_iv=()):
    """Вогель-критерий Рпл (v10.5): изменение Qж при ПОСТОЯННОЙ огибающей Рпр (допуск
    rplq_ip_tol_frac). Обратный пересчёт Рпл по Вогелю при постоянном Кпрод J:
    ...
    при размахе огибающей Рпр в окне <= rplq_ip_tol_frac. Тренд >= rplq_min_trend_d.
    ...
    for W, thr in ((15, P['rplq_chg15_frac']), (30, P['rplq_chg30_frac'])):
        prev = pr.shift(W)
        rel = (pr - prev) / prev
        ipmax = df['ip'].rolling(W + 1, min_periods=W).max()
        ipmin = df['ip'].rolling(W + 1, min_periods=W).min()
        stable = (ipmin > 0) & ((ipmax / ipmin - 1) <= P['rplq_ip_tol_frac'])
        fdn |= ((rel <= -thr) & stable).fillna(False)
        fup |= ((rel >= thr) & stable).fillna(False)
    for d in df.index:
        if in_any(d, nur_iv):
            fdn.loc[d] = False; fup.loc[d] = False
    mind = P['rplq_min_trend_d']
    def _ip_const(a, e):
    ...
    dn = [(a, e) for a, e in _flag_to_iv(fdn) if (e - a).days + 1 >= mind and _ip_const(a, e)]
    up = [(a, e) for a, e in _flag_to_iv(fup) if (e - a).days + 1 >= mind and _ip_const(a, e)]
    return dn, up
...
def det_neopt_cyclic(ctx):
    """Неопт. режим — критерий периодического фонда (v10.4).
    Гейт: циклическая скважина ИЛИ > neopt_stop_n остановок за скользящие neopt_stop_win_d сут.
    Признак суток (ИЛИ):
      ...
      B. Рпр выше Рзатр + neopt_ip_over_cas_bar (высокий подпор над затрубным — можно отбирать больше).
    ...
    ip = ip[ip > 0]
    days = pd.date_range(ctx.t0.normalize(), ctx.t1.normalize(), freq='D')
    # ── гейт периодичности: циклич. режим или плотность остановок ──
    if getattr(ctx, 'cyclic', False):
        gate = pd.Series(True, index=days)
    else:
        onsets = pd.Series(1, index=pd.DatetimeIndex([s0 for s0, _ in ctx.stops]))
        cnt = (onsets.resample('1D').sum().reindex(days, fill_value=0)
               .rolling(P['neopt_stop_win_d'], min_periods=1).sum())
        gate = cnt > P['neopt_stop_n']
    if not gate.any():
        return []
    ...
    # ── B: Рпр выше Рзатр + порог ──
    if CAS in ctx.tele:
        cas = pd.to_numeric(ctx.tele[CAS], errors='coerce').dropna()
        ipd = ip.resample('1D').median()
        casd = cas.resample('1D').median()
        both = pd.concat([ipd, casd], axis=1, keys=['ip', 'cas']).dropna()
        hi = both.index[both['ip'] > both['cas'] + P['neopt_ip_over_cas_bar']]
        flag.loc[flag.index.intersection(hi)] = True
...
def det_umch(ctx, rptch_iv=None, periodic_iv=None):
    ...
    while i < n - 1:
        ...
        _post = fraw[(fraw.index > e_run) & (fraw.index <= e_run + pd.Timedelta(days=P['uvch_hold_d']))]
        _held = (len(_post) == 0) or (float(_post.median()) <= base - P['uvch_rise_hz'])
        if (dur_d >= P['umch_min_run_d'] and net >= P['uvch_rise_hz'] and not end_low and _held
    ...
    if getattr(ctx, 'cyclic', False):
        ...
        if len(fd) > P['uvch_slow_win']:
            ...
            out += [(_s, _e) for _s, _e in merge_iv(_flag_to_iv(flag), gap=pd.Timedelta(days=10)) if (_e - _s).days >= P['uvch_slow_min_d']]
    # v10.3: ОГИБАЮЩАЯ частоты на периодическом/РПТЧ/циклическом фонде — осцилляции не УМЧ,
    # но устойчивое снижение верхней огибающей = УМЧ (Ic_370 июл-сен 2025, Ic_805 ноя-дек 2025).
    ...
    if getattr(ctx, 'cyclic', False) or _rptch or (periodic_iv or []):
        if len(f) > P['umch_env_win']:
            fullw = f.reindex(pd.date_range(f.index.min(), f.index.max(), freq='D')).interpolate(limit=10)
            env = fullw.rolling(7, min_periods=3).quantile(P['umch_env_q'])
            edrop = env.shift(P['umch_env_win']) - env
            eflag = pd.Series((edrop >= P['umch_env_net']).fillna(False), index=env.index)
            out += [(_s, _e) for _s, _e in merge_iv(_flag_to_iv(eflag), gap=pd.Timedelta(days=10))
                    if (_e - _s).days >= P['uvch_slow_min_d']]
    return _clear_stops(merge_iv(out), ctx, P['uvch_stop_clear_d'])
...
def label_well(tele, vsp, P=PARAMS, gdi_events=None):
    ...
    _neopt = list(det_deopt(ctx, res['НУР'])) + list(det_neopt_cyclic(ctx))
    _neopt = merge_iv(_neopt, gap=pd.Timedelta(days=P['neopt_merge_gap_d']))
    ...
    for _k in ('ВГФ', 'Рост ГФ', 'Снижение ГФ', 'Деградация ЭЦН'):   # v10.4: Неопт. режим — своя склейка (2 сут)
        if res.get(_k):
    ...
    def _vogel_priority(vg):
        ...
        for _s0, _e0 in vg:
            ...
            kept += [(_a, _b) for _a, _b in _pieces
                     if (_b - _a).days + 1 >= P['rplq_min_trend_d']]
        return kept
    if _vg_dn:
        res['Снижение Рпл'] = merge_iv(list(res['Снижение Рпл']) + _vogel_priority(_vg_dn))
    if _vg_up:
        res['Рост Рпл'] = merge_iv(list(res['Рост Рпл']) + _vogel_priority(_vg_up))
...
def explain_episode(ctx, label, s, e, tier=""):
    ...
    if label == "Неопт. режим":
        ...
        return "Неоптимальный режим: рост Рпр за цикл остановки более порога либо Рпр выше Рзатр (периодический фонд), либо рост Рпр при стабильном дебите."
    if label == "ВГФ":
