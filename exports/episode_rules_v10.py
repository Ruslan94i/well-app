# -*- coding: utf-8 -*-
"""
episode_rules_v9_3.py — episode_rules_v10.py — v10: газовые категории (ВГФ/Рост ГФ/Снижение ГФ);
...
def det_umch(ctx, rptch_iv=None):
    ...
    while i < n - 1:
        ...
        if dur_d >= P['uvch_min_run_d'] and net >= P['uvch_rise_hz'] and not end_low:
            out.append((s_run, e_run + pd.Timedelta(hours=23)))
        i = max(j, i + 1)
    if getattr(ctx, 'cyclic', False):
        fd = ctx.wd[F].dropna()
