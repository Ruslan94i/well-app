...
MODEL_VERSION = "episode_rules_v10_1"
...
def det_wcut(ctx):
    """Рост/Снижение обводнённости по СУТОЧНОМУ изменению обводнённости.
    Обводнённость = soft-sensor wct_pred (доля воды в жидкости, %), вход — работа
    алгоритма обводнённости (full_inference_water_cut). Условие:
      Обв(тек) - Обв(вчера) > +wct_step_pp  -> Рост обводнённости
      Обв(тек) - Обв(вчера) < -wct_step_pp  -> Снижение обводнённости
    Возвращает (рост_iv, снижение_iv); близкие эпизоды склеиваются (wct_merge_d)."""
    P = ctx.P
    ...
    if w.empty:
        return [], []
    d = w.resample('1D').median().dropna()      # суточная обводнённость
    if len(d) < 2:
        return [], []
    idx = list(d.index); vals = list(d.values); rost = []; sniz = []
    hold = pd.Timedelta(days=P['wct_hold_d'])
    for k in range(1, len(idx)):
        delta = vals[k] - vals[k - 1]
        if abs(delta) <= P['wct_step_pp']:
            continue
        pre = vals[k - 1]
        post = d[(d.index > idx[k]) & (d.index <= idx[k] + hold)]
        if delta > P['wct_step_pp']:
            # удержание: обводнённость осталась ПОВЫШЕННОЙ (не вернулась) — иначе транзиент
            if len(post) == 0 or float(post.median()) >= pre + P['wct_step_pp']:
                rost.append((idx[k - 1], idx[k]))
        else:
            # удержание: обводнённость осталась ПОНИЖЕННОЙ
            if len(post) == 0 or float(post.median()) <= pre - P['wct_step_pp']:
                sniz.append((idx[k - 1], idx[k]))
    wk = pd.Timedelta(days=P['wct_merge_d'])
    return merge_iv(rost, gap=wk), merge_iv(sniz, gap=wk)
...
def score_episode(ctx, ep_type, start, end):
    ...
    # ─── Снижение Кпрод ──────────────────────────────────────────────────
        ...
        return dict(score=0.8 if tier == 'high' else 0.55, tier=tier, signals={'ql_pts': int(len(ql))})
